...
async function apolloRequest<T>(
  ...
  if (res.status >= 500)
    throw new ApolloError("server_error", "Apollo is unavailable.", res.status);
...
export async function validateApolloKey(apiKey: string): Promise<ApolloProfile> {
  ...
  const profile = await apolloRequest<Record<string, unknown>>(
    ...
    if (
      ...
      (err.code === "invalid_request" || err.code === "forbidden")
    ) {
...
export async function probeApolloCapabilities(apiKey: string): Promise<CapabilityReport[]> {
  ...
  out.push({
    ...
    state: "not_tested",
    detail:
      "Not probed: creating a contact writes to your Apollo account. Verified during the end-to-end test run.",
  });
  out.push({
    id: "create_sequence",
    ...
    state: "not_tested",
    detail:
      "Not probed: creating a sequence writes to your Apollo account. Verified during the end-to-end test run.",
  });
  out.push({
    id: "add_to_sequence",
    ...
    state: "not_tested",
    detail:
      "Requires contact IDs, a sequence ID and a sending email account. Verified during the end-to-end test run.",
  });
...
export async function createContact(
  ...
  const res = await apolloRequest<{ contact?: { id?: string } }>(
    apiKey,
    "POST",
    "/api/v1/contacts",
    {
...
export async function addContactsToSequence(
  ...
  return {
    added: Array.isArray(res.data?.contacts) ? res.data!.contacts!.length : input.contactIds.length,
  };
}
