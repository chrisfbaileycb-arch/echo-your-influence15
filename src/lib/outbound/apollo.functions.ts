  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      await recordStep(
        ...
        "key_validated",
        "passed",
        "auth/health + users/api_profile succeeded.",
      );
...
export const sourceLeads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (rows.length) {
      ...
      if (error)
        return { ok: false as const, message: "Could not persist leads.", inserted: 0, seen };
    }
    ...
    if ((after ?? 0) > 0)
      await recordStep(orgId, "lead_persisted", "passed", `${after} leads stored.`);
...
export const qualifyLeads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (results.length)
      await recordStep(orgId, "qualified", "passed", `${results.length} leads scored.`);
...
export const enrichLeads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!eligible.length)
      return {
        ok: false as const,
        message: "All selected leads were rejected by qualification.",
        enriched: 0,
      };
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const providerSequenceId =
        data.existing_provider_sequence_id ??
        (await apollo.createSequence(key, seq.name)).sequenceId;
      ...
      await recordStep(
        ...
        "sequence_created_or_selected",
        "passed",
        "Sequence available in Apollo (inactive).",
      );
      return {
        ok: true as const,
        providerSequenceId,
        reused: Boolean(data.existing_provider_sequence_id),
      };
    } catch (err) {
      const message = describe(err);
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!sendable.length)
      return {
        ok: false as const,
        message:
          "No eligible leads: they need a verified email and must not be rejected, bounced or unsubscribed.",
      };
    ...
    if (!contactIds.length)
      return { ok: false as const, message: "Apollo returned no contact ids." };
    await recordStep(
      orgId,
      "contact_deduped_or_created",
      "passed",
      `${contactIds.length} contacts ready (run_dedupe=true).`,
    );
    ...
    await recordStep(
      ...
      "enrollment_succeeded",
      "passed",
      `${contactIds.length} contacts added to the sequence (paused).`,
    );
