...
const AI_GATEWAY =
  process.env.AI_GATEWAY_URL || process.env.OPENAI_BASE_URL || "https://api.openai.com/v1";
const MODEL = "google/gemini-3.6-flash";

async function aiJson(system: string, user: string): Promise<Record<string, unknown>> {
  const key =
    process.env.OPENAI_API_KEY || process.env.GEMINI_API_KEY || process.env.LOVABLE_API_KEY;
  if (!key) throw new Error("AI API Key is not configured (OPENAI_API_KEY or GEMINI_API_KEY).");
...
export async function generateSequenceDraft(
  ...
  return rows
    .map(
      (s) =>
        s as { step_number?: unknown; subject?: unknown; body?: unknown; delay_days?: unknown },
    )
    .map((s, i) => ({
...
"notes":[string]}`,
  ...
  const cleaned = rows
    ...
    .map((r) => {
      const share =
        typeof r["budget_share"] === "number" ? Math.max(0, Math.round(r["budget_share"])) : 0;
      const priority = str(r["priority"], "support");
      const engine = str(r["engine"], "avatar");
      return {
        platform: str(r["platform"]),
        priority: (["primary", "support", "test"].includes(priority)
          ? priority
          : "support") as ChannelRecommendation["priority"],
        format: str(r["format"]),
        engine: (["avatar", "broll", "image", "text"].includes(engine)
          ? engine
          : "avatar") as ChannelRecommendation["engine"],
  ...
  return {
    ...
    notes: Array.isArray(raw["notes"])
      ? (raw["notes"] as unknown[]).filter((x): x is string => typeof x === "string")
      : [],
