...
export const setActiveOutboundProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabaseAdmin
      .from("org_settings")
      .upsert({ org_id: orgId, active_outbound_provider: data.provider }, { onConflict: "org_id" });
  ...
  .handler(async ({ context }) => {
    ...
    if (!ayrshareConfigured()) {
      return {
        ...
        message: "Staged / Dry Run Mode — Connect Ayrshare Key to Activate Live Execution",
      };
    }
    if (!crypto.encryptionAvailable()) {
      return {
    ...
    if (existing?.profile_ref_ciphertext) {
      ...
      if (!created.ok)
        return { ok: false as const, blocker: created.blocker, message: created.message };
