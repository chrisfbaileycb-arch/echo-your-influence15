...
export const createWorkflow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if ((count ?? 0) >= MAX_ACTIVE_CAMPAIGNS) {
      throw new Error(
        `Campaign limit reached: maximum ${MAX_ACTIVE_CAMPAIGNS} active campaigns allowed at once. Delete or archive an existing campaign before starting a new one.`,
      );
    }
...
export const deleteWorkflow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { orgId, db } = await ctx(context.userId);
    await db.from("product_briefs").delete().eq("workflow_id", data.id).eq("org_id", orgId);
    await db.from("gtm_strategies").delete().eq("workflow_id", data.id).eq("org_id", orgId);
    await db.from("content_packs").delete().eq("workflow_id", data.id).eq("org_id", orgId);
    await db.from("outbound_campaigns").delete().eq("workflow_id", data.id).eq("org_id", orgId);
  ...
  .handler(async ({ data, context }) => {
    ...
    const [brief, strategy, pack, outbound, seq] = await Promise.all([
      ...
      db
        .from("sequences")
        .select(
          "id, name, provider_sequence_id, active, sequence_steps(id, step_number, subject, body, delay_days)",
        )
        .eq("org_id", orgId)
    ...
    const sequences = (seq.data ?? []).filter((s) => (outboundId ? true : false));
...
export const attachCampaignKit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!campaign || campaign.user_id !== context.userId)
      throw new Error("Campaign kit not found.");
    await db
      .from("campaign_workflows")
...
async function buildAnalytics(orgId: string, workflowId: string) {
  ...
  const links = wf.campaign_id
    ? ((
        await db
          ...
          .eq(
            "product_id",
            (await db.from("campaigns").select("product_id").eq("id", wf.campaign_id).maybeSingle())
  ...
  return {
    ...
    outbound: {
      ...
      source:
        "Apollo pipeline records in this database. Send/reply counts appear only when a validated provider returns them.",
    },
    attribution: {
