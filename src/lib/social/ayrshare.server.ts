...
async function call<T>(
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      return { ok: false, blocker: "PROVIDER_ERROR", message: "Ayrshare request timed out." };
    }
...
export const ayrshareAdapter: SocialAdapter = {
  ...
  async linkingUrl(profileRef, platforms) {
    ...
    const res = await call<{ url?: string; token?: string; expiresIn?: number }>(
