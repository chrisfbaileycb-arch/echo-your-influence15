...
import { listCalendarSlots, createCalendarSlot, type CalendarSlot } from "@/lib/calendar.functions";
import {
  VIEW_LABEL,
  WEEKDAY_HEADERS,
  daysInRange,
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="max-w-2xl text-muted-foreground">
          Every day owns its own page — engine choice, hook, 15–30s script, video prompt, caption,
          two hashtags and the affiliate disclosure. Posting happens on your phone through the share
          sheet to TikTok, Reels, Shorts or Facebook.
        </p>
      </header>
        ...
        <div className="min-w-[640px]">
          <div className="grid grid-cols-7 gap-2 pb-2">
            {WEEKDAY_HEADERS.map((d) => (
              <div
                key={d}
                className="text-center text-xs font-mono uppercase tracking-widest text-muted-foreground"
              >
                {d}
              </div>
            ))}
