  ...
  head: () => ({
    meta: [
      { title: "Campaign workspace — Echo Your Influence" },
      {
        name: "description",
        content:
          "Resumable campaign: brief, strategy, content, outbound pipeline, publishing and a fact-based report.",
      },
    ],
  }),
});
...
function OutboundStep({ id, data, refresh }: StepProps) {
  ...
  const [seqSteps, setSeqSteps] = useState<
    Array<{ step_number: number; subject: string; body: string; delay_days: number }>
  >([]);
  ...
  const [previewRows, setPreviewRows] = useState<
    Array<{ name: string | null; title: string | null; organization: string | null }>
  >([]);
    ...
    <div className="max-w-4xl space-y-4">
      <Card className="space-y-3 p-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                run(
                  ...
                  () => {
                    status.refetch();
                    toast.success("Apollo key validated");
                  },
                )
              }
            >
            ...
            <Button
              ...
              onClick={() =>
                run(
                  () => probe({}),
                  (r) => setCapabilities(r.capabilities),
                )
              }
            >
              Probe capabilities
        ...
        {st?.truthStatus !== "working" ? (
          <p className="text-xs text-muted-foreground">
            "Working" is only set automatically once every step of a real end-to-end test run
            passes. No manual override exists.
          </p>
        ) : null}
        {capabilities.length ? (
      ...
      <Card className="space-y-3 p-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() =>
              run(
                () => saveFilters({ data: { workflow_id: id, filters } }),
          ...
          <Button
            size="sm"
            data-testid="source-leads"
            onClick={() =>
              run(
                () => source({ data: { workflow_id: id, filters, pages: 1, perPage: 10 } }),
                (r) => {
                  refresh();
                  toast.success(`${r.inserted} new leads (${r.seen} returned)`);
                },
              )
            }
          >
          ...
          <Button
            ...
            onClick={() =>
              run(
                () => qualify({ data: { workflow_id: id } }),
                (r) => {
                  refresh();
                  toast.success(`${r.scored} leads scored`);
                },
              )
            }
          >
        ...
        {previewRows.length ? (
          <ul className="space-y-1 text-xs text-muted-foreground">
            {previewRows.map((p, i) => (
              <li key={i}>
                {p.name} — {p.title} @ {p.organization}{" "}
                <span className="opacity-60">(no email until enrichment)</span>
              </li>
            ))}
          </ul>
        ) : null}
      ...
      <Card className="space-y-3 p-6">
        ...
        <div className="max-h-72 space-y-1 overflow-auto text-xs">
          ...
          {!data.leads.length ? (
            <p className="text-muted-foreground">No leads sourced yet.</p>
          ) : null}
        </div>
        <label className="flex items-start gap-2 text-xs">
          <Checkbox
            checked={confirmCredits}
            onCheckedChange={(v) => setConfirmCredits(Boolean(v))}
          />
          <span>
        ...
        <Button
          ...
          onClick={() =>
            run(
              ...
              (r) => {
                refresh();
                toast.success(`${r.enriched} emails found`);
              },
            )
          }
        >
          ...
          <Button
            ...
            onClick={() =>
              run(
                ...
                () => {
                  refresh();
                  toast.success("Sequence saved locally");
                },
              )
            }
          >
          ...
          <Button
            ...
            onClick={() =>
              run(
                ...
                () => {
                  refresh();
                  toast.success("Sequence created in Apollo (inactive)");
                },
              )
            }
          >
        ...
        <Button
          ...
          onClick={() =>
            run(
              ...
              (r) => {
                refresh();
                toast.success(r.note ?? "Enrolled (paused)");
              },
            )
          }
        >
...
function AnalyticsStep({ id }: { id: string }) {
  ...
  const q = useQuery({
    ...
    queryFn: () => get({ data: { workflow_id: id } }),
  });
