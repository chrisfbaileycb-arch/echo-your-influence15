...
function ContentPage() {
  const { platform } = useSearch({ from: "/_authenticated/content" });
  const navigate = useNavigate();

  return (
    <ModuleShell
      title="Content by platform"
      description="The hooks, scripts and captions for this campaign, plus platform-by-platform continuous execution workflows. Select a platform to view its complete script, media asset, and publishing step."
    >
      {({ id, data, refresh }) => (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2 border-b border-border pb-3">
            {PLATFORM_TABS.map((tab) => {
              const active = (platform || "all") === tab.id;
              return (
                <Button
                  key={tab.id}
                  size="sm"
          ...
          <ContentStep id={id} data={data} refresh={refresh} selectedPlatform={platform || "all"} />
          <WeekBreakdown />
        </div>
      )}
    </ModuleShell>
