...
const ENGINE_STEP: Record<string, { label: string; href: string; cta: string; note: string }> = {
  avatar: {
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-1">
        ...
        <p className="text-muted-foreground">
          Each slot below is one post. Fill in the product and notes, then generate the prompt set
          for that slot.
        </p>
      </header>
      ...
      <div className="space-y-4">
        {(slotsQuery.data ?? []).map((slot) => (
          <SlotCard key={slot.id} slot={slot} date={date} products={productsQuery.data ?? []} />
        ))}
      </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {draft.platforms.map((p) => {
              ...
              const plan = draft.platform_plans.find((x) => x.platform === p) ?? {
                platform: p,
                hook: draft.hook,
                script: draft.script,
                caption: draft.caption,
